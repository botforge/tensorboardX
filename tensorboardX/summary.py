...
def video(tag, tensor, fps=4, as_np_framebuffer=False):
    tag = _clean_tag(tag)
    if not as_np_framebuffer:
        tensor = make_np(tensor)
        ...
        video = make_video(tensor, fps, as_np_framebuffer=True)
    return Summary(value=[Summary.Value(tag=tag, image=video)])


def make_video(tensor, fps, as_np_framebuffer=False):
    try:
